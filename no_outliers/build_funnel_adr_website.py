...
Usage
=====
    python build_funnel_adr_website.py \\
        ...
        --source funnel_quoted_adr_v4_verified_1_fixed_1.html \\
...
IMPORTANT — --source must be the ORIGINAL template, never a previous output
=============================================================================
--source has to be the pristine funnel_quoted_adr_v4 HTML file you started
with. It must NOT be a file this script (or its predecessor,
render_funnel_website.py) already wrote out — a `funnel_adr_rebuilt*.html`.
The templating step does surgical text-replacement keyed to markers that
exist exactly once in the original page; running it against an
already-rebuilt page means those markers get matched a second time INSIDE
content this script already inserted, which corrupts the page (duplicated
`<div class="callout">` blocks, a duplicate `const EXCL = ...` declaration
that throws `Identifier 'EXCL' has already been declared` and silently
kills every dynamic chart on the page). The script now detects this case up
front and refuses to run rather than producing a broken page — see
render_html()'s guard clause below. If you only need to re-render the page
(e.g. because you pointed --source at the wrong file last time, but the
pipeline run itself was fine), you don't need to retrain: rerun with
--from-json pointed at the funnel_payload_rebuilt.json from that earlier
run, and this time pass the correct --source. That skips straight to
rendering and takes seconds instead of minutes.

    python build_funnel_adr_website.py \\
        --from-json funnel_payload_rebuilt.json \\
        --source funnel_quoted_adr_v4_verified_1_fixed_1.html \\
        --out funnel_adr_rebuilt.html
"""
import argparse
...
# Marker this script itself injects into every page it renders (see the
# `const EXCL = ...` block in step 6 below). If this is already present in
# --source, that file is a PREVIOUS OUTPUT of this script, not the original
# template — templating onto it a second time is what produces the
# "Identifier 'EXCL' has already been declared" crash and duplicated
# callout divs. Caught up front in render_html() so the script fails fast
# with a clear message instead of writing a broken page.
ALREADY_REBUILT_MARKER = "const EXCL = POINTS.is_excluded_from_training"

BAD_SOURCE_MSG = """\
--source looks like a file THIS SCRIPT (or render_funnel_website.py) already
wrote out, not the original funnel_quoted_adr_v4 template — it already
contains the marker this script injects ({marker!r}).

Templating onto an already-rebuilt page corrupts it: markers this script
looks for get matched a second time inside content it already inserted,
producing a duplicated <div class="callout"> and a duplicate `const EXCL`
declaration that throws "Identifier 'EXCL' has already been declared" in
the browser and silently kills every dynamic chart on the page.

Fix: point --source at the ORIGINAL, untouched funnel_quoted_adr_v4 HTML
file (the one you started with before any rebuild), not at any
funnel_adr_rebuilt*.html this script produced. If you don't have the
original handy but do have the JSON payload from a prior run, you don't
need to retrain — rerun with --from-json <that payload> and the correct
--source; it'll skip straight to rendering.
"""
...
def get_between(content, start_marker, end_marker, start_after=0):
    ...
    if i == -1:
        raise ValueError(
            f"start marker not found: {start_marker!r}\n"
            "This usually means --source is not the pristine original "
            "funnel_quoted_adr_v4 template (for example, it may already be "
            "a funnel_adr_rebuilt*.html this script produced earlier). "
            "Point --source at the original file.")
    j = content.find(end_marker, i)
    if j == -1:
        raise ValueError(
            f"end marker not found: {end_marker!r}\n"
            "This usually means --source is not the pristine original "
            "funnel_quoted_adr_v4 template. Point --source at the original file.")
...
def render_html(source_path, payload):
    ...
    if ALREADY_REBUILT_MARKER in content:
        raise SystemExit(BAD_SOURCE_MSG.format(marker=ALREADY_REBUILT_MARKER))
  ...
  <div class="callout">"""
    ...
    def replace_note(old_key_note_start, new_note):
        ...
        if i0 == -1:
            raise ValueError(
                f"note anchor not found: {old_key_note_start[:60]!r}\n"
                "This usually means --source is not the pristine original "
                "funnel_quoted_adr_v4 template. Point --source at the original file.")
...
def _validate_rendered(content):
    """Defense in depth, on top of the up-front ALREADY_REBUILT_MARKER guard:
    sanity-check the actually-rendered output before we let main() write it
    to disk, so a subtler templating bug fails loudly here instead of
    shipping a silently-broken page. Checks the specific things step 6's
    text-replacement could duplicate if run twice."""
    problems = []
    n_excl_decl = content.count("const EXCL = POINTS.is_excluded_from_training")
    if n_excl_decl != 1:
        problems.append(f"expected exactly 1 'const EXCL = ...' declaration, found {n_excl_decl}")
    n_payload_scripts = content.count('<script id="funnel-data" type="application/json">')
    if n_payload_scripts != 1:
        problems.append(f"expected exactly 1 funnel-data JSON <script> tag, found {n_payload_scripts}")
    n_h1 = content.count("<h1>Quoted ADR pruning funnel")
    if n_h1 != 1:
        problems.append(f"expected exactly 1 <h1>Quoted ADR pruning funnel..., found {n_h1}")
    if problems:
        raise SystemExit(
            "Rendered output failed a sanity check and was NOT written to disk:\n  - "
            + "\n  - ".join(problems)
            + "\nThis almost always means --source was already a rebuilt page rather than "
              "the original template. Point --source at the original funnel_quoted_adr_v4 "
              "file and try again."
        )
...
def main():
    ...
    ap.add_argument("--transient-data", default=None,
                     help="Raw transient-demand CSV (e.g. Nexus_Transient_Demand_v2.csv). Optional — "
                          "omit to use whatever tr_* columns --data already has. Ignored with --from-json.")
    ap.add_argument("--from-json", default=None,
                     help="Skip the pipeline entirely and render straight from a funnel_payload_rebuilt.json "
                          "written by a previous run (--out-json). Use this to re-render after fixing "
                          "--source without waiting through the Optuna search again. When given, "
                          "--data/--pipeline/--transient-data/--out-model are not needed and are ignored.")
    ap.add_argument("--source", required=True,
                     help="The ORIGINAL funnel_quoted_adr_v4 HTML file to template onto — never a "
                          "funnel_adr_rebuilt*.html this script already produced (see module docstring).")
    ...
    ap.add_argument("--out-model", default=None, help="Optional: also write the trained model bundle (.pkl). "
                                                        "Ignored with --from-json (no model was (re)trained).")
    ...
    if args.from_json:
        print(f"Loading existing payload from {args.from_json} (skipping the pipeline run)...")
        with open(args.from_json, encoding="utf-8") as fh:
            payload = json.load(fh)
        bundle = None
    else:
        if not args.data or not args.pipeline:
            ap.error("--data and --pipeline are required unless --from-json is given")
    ...
    if args.out_model:
        if bundle is None:
            print(f"Skipping {args.out_model}: no model was (re)trained this run (--from-json was used).")
        else:
            with open(args.out_model, "wb") as fh:
                pickle.dump(bundle, fh)
            print(f"Wrote {args.out_model}")

    if args.out_json and not args.from_json:
