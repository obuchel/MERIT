...
(fig_g3_stability.png) and Figure S12 (supp_g3_direction_by_year.png):

    supp_g3_circulation_own.png     (Figure S3)
    supp_g3_program_shared.png      (Figure S6)
    supp_g3_program_own.png         (Figure S7)
    supp_g3_poverty_shared.png      (Figure S10)
    supp_g3_poverty_own.png         (Figure S11)
...
shown in a map here." But mgwr_cross_sectional_g3.json's 2024 entry has full
...
captured above, just drop '2024' from YEARS_G3 below and nothing else needs
...
Requires (same directory): mgwr_cross_sectional_g3.json, us_borders.py,
...
IN = "mgwr_cross_sectional_g3.json"
data = json.load(open(IN))
names = data['names']
YEARS_G3 = ['2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019', '2022', '2023', '2024']
print("years used:", YEARS_G3, f"({len(YEARS_G3)} panels)")
...
def year_arrays(varname):
    """Returns {year: (xs, ys, vals)} for one predictor across YEARS_G3."""
...
def make_grid(varname, label, scale, outfile):
    ...
    scale: 'shared' -> one vmin/vmax (symmetric, based on max |coef| across
           all YEARS_G3) with a single colorbar for the whole figure.
    ...
    for i, yr in enumerate(YEARS_G3):
        ax = axes_flat[i]
    ...
    for j in range(len(YEARS_G3), NROWS * NCOLS):
    ...
    # figures (e.g. plot_g3_direction_by_year.py): a short bold claim, and a
    ...
    fig.suptitle(f"{label}, grade 3", fontsize=21, fontweight='bold',
